#!/usr/bin/env python3
"""
Test the transcript Parse API endpoints.

Usage:
    # First create a test user and get a token
    python scripts/create_test_user.py

    # Then run the transcript API tests
...
def test_upload(client: httpx.Client, api_url: str, pdf_path: Path, token: str) -> Optional[str]:
    """Test the transcript parse upload endpoint."""
    print(f"\n[2/4] Testing transcript PDF upload: {pdf_path}")
...
def test_job_status(client: httpx.Client, api_url: str, job_id: str, token: str) -> bool:
    """Test transcript job status endpoint and poll until complete."""
    print(f"\n[3/4] Testing transcript job status: {job_id}")
...
def test_latest(client: httpx.Client, api_url: str, token: str) -> bool:
    """Test latest transcript endpoint."""
    ...
    if response.status_code == 404:
        print("  ! No transcript found (expected if job has not completed)")
        print(f"    Response: {response.text}")
        return True
...
def main():
    parser = argparse.ArgumentParser(description="Test the transcript Parse API")
    parser.add_argument(
        "--api-url",
    ...
    print("=" * 60)
    print("TRANSCRIPT PARSE API TEST")
    print("=" * 60)
