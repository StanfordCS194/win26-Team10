...
from api.pipeline.steps import ResumeParseStep
...
async def process_job(job: dict) -> None:
    ...
    job_type = job.get("job_type", "transcript")
    ...
    try:
        ...
        if job_type == "resume":
            ...
            resume_analysis_path = output_dir / "resume_analysis.json"

            if storage_path and resume_path.exists():
                await asyncio.to_thread(
                    upload_bytes,
                    content=resume_path.read_bytes(),
                    dest_path=f"{storage_path}/resume.json",
                    content_type="application/json",
                )
            if storage_path and resume_analysis_path.exists():
                await asyncio.to_thread(
                    upload_bytes,
                    content=resume_analysis_path.read_bytes(),
                    dest_path=f"{storage_path}/resume_analysis.json",
                    content_type="application/json",
                )
            ...
            if user_id:
                detail_data = {
                    "resume_raw": artifacts.resume_raw,
                    "resume_analysis": artifacts.resume_analysis,
                }
                logger.info(f"Upserting resume detail for {user_id}")
